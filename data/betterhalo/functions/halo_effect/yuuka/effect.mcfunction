...
# yuuka_target = falldistance >= 3
#   ? world.entities.get(distance=..1.7,type=!#betterhalo:block_entity,type=!player)
#   : []
...
scoreboard players set @s clipboad1 8
...
execute as @e[tag=yuuka_target] run execute at @e run scoreboard players operation @s health -= @s[scores={falldistance=3..},distance=..1.7] clipboad2
...
# visual effects
execute at @s[scores={falldistance=1..}] unless block ~ ~-1 ~ air run particle dust 0.71 0.71 0.71 1 ~ ~ ~ 0.2 0.001 0.2 0 10 normal
execute at @s[scores={falldistance=3..}] unless block ~ ~-1 ~ air run particle minecraft:firework ~ ~ ~ 0.00001 0.0000001 0.00001 0.1 20 normal
execute as @s[scores={falldistance=3..}] at @s unless block ~ ~-1 ~ air run playsound minecraft:entity.iron_golem.step player @s ~ ~-2 ~ 7 0.7
execute as @s[scores={falldistance=1..3}] at @s unless block ~ ~-1 ~ air run playsound minecraft:block.anvil.step player @s ~ ~-2 ~ 1 0.6
...
## Q.E.D
# continue next "tick" if skill is aready active
execute as @s[tag=creating_shield] run function betterhalo:halo_effect/yuuka/create_shield

execute store result score @s health run data get entity @s Health
execute as @s[tag=!creating_shield,scores={health=..5,cooldown_a=0}] run tag @s add shield_active

execute as @s[tag=!creating_shield,tag=shield_active] run function betterhalo:halo_effect/yuuka/create_shield
