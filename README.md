...
Download this repo as a zip file (**make sure to choose the correct branch with version matching your Minecraft version**) and rename it to `betterhalo` (keep the extension as `.zip`)
...
then paste the downloaded `zip` file there.
...
# or
.minecraft/saves/YOUR WORLD NAME/datapacks/betterhalo.zip
```

> You can use this datapack in a server ONLY IF you are the one hosting it.
> Datapacks can be installed in the server the same way it's done on a single player world, by pasting datapack in the world's `datapacks` folder.
